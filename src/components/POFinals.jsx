...
function POFinals({
  ...
  useEffect(() => {
    const updateData = () => {
      setNewMatchups(() => {
        return POFinalsData.map((matchup) => ({
      ...
      const teamsToSet = teamData.filter(
        (team) => team.stage === "PlayoffsFinals"
      );

      setSelectableTeams(teamsToSet);
    };
    updateData();
  }, [POFinalsData, teamData]);
          ...
          <tfoot>
            <tr>
              <td colSpan="3" style={{ textAlign: "center" }}>
                <button
                  onClick={() => {
                    updateMatchups(newMatchups, "Playoff", "Finals");
                    updateWinner(newMatchups, "Playoff", "Finals", "Victor");
                  }}
                >
                  Update
                </button>

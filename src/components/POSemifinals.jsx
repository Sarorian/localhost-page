...
function POSemifinals({
  ...
  useEffect(() => {
    const updateData = () => {
      setNewMatchups(() => {
        return POSemifinalsData.map((matchup) => ({
      ...
      const teamsToSet = teamData.filter((team) => team.stage === "playoffs");

      setSelectableTeams(teamsToSet);
    };
    updateData();
  }, [POSemifinalsData, teamData]);
  ...
  return (
    <table className="bracket-table">
      ...
      <tbody>
        {POSemifinalsData &&
          POSemifinalsData.map((matchup, index) => (
          ...
          <tfoot>
            <tr>
              <td colSpan="3" style={{ textAlign: "center" }}>
                <button
                  onClick={() => {
                    updateMatchups(newMatchups, "Playoff", "Semifinals");
                    updateWinner(
                      newMatchups,
                      "Playoff",
                      "Semifinals",

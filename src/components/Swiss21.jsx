...
function Swiss21({
  ...
  useEffect(() => {
    const updateData = () => {
      setNewMatchups(() => {
        return swiss21Data.map((matchup) => ({
      ...
      const teamsToSet = teamData.filter(
        (team) => team.record.wins === 2 && team.record.losses === 1
      );

      setSelectableTeams(teamsToSet);
    };
    updateData();
  }, [swiss21Data, teamData]);
          ...
          <tfoot>
            <tr>
              <td colSpan="3" style={{ textAlign: "center" }}>
                <button
                  onClick={() => {
                    updateMatchups(newMatchups, "Swiss", "2-1");
                    updateWinner(newMatchups, "Swiss", "2-1", "playoffs");
                  }}
                >
                  Update
                </button>

...
function COFinals({
  ...
  useEffect(() => {
    const updateData = () => {
      setNewMatchups(() => {
        return COFinalsData.map((matchup) => ({
      ...
      const teamsToSet = teamData.filter(
        (team) => team.stage === "ConsolationFinals"
      );

      setSelectableTeams(teamsToSet);
    };
    updateData();
  }, [COFinalsData, teamData]);
  ...
  return (
    <table className="bracket-table">
      ...
      <tbody>
        {COFinalsData &&
          COFinalsData.map((matchup, index) => (
          ...
          <tfoot>
            <tr>
              <td colSpan="3" style={{ textAlign: "center" }}>
                <button
                  onClick={() => {
                    updateMatchups(newMatchups, "Consolation", "Finals");
                    updateWinner(
                      newMatchups,
                      "Consolation",
                      "Finals",

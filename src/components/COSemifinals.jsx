...
function COSemifinals({
  ...
  useEffect(() => {
    const updateData = () => {
      setNewMatchups(() => {
        return COSemifinalsData.map((matchup) => ({
      ...
      const teamsToSet = teamData.filter(
        (team) => team.stage === "ConsolationSemifinals"
      );

      setSelectableTeams(teamsToSet);
    };
    updateData();
  }, [COSemifinalsData, teamData]);
  ...
  return (
    <table className="bracket-table">
      ...
      <tbody>
        {COSemifinalsData &&
          COSemifinalsData.map((matchup, index) => (
          ...
          <tfoot>
            <tr>
              <td colSpan="3" style={{ textAlign: "center" }}>
                <button
                  onClick={() => {
                    updateMatchups(newMatchups, "Consolation", "Semifinals");
                    updateWinner(
                      newMatchups,
                      "Consolation",
                      "Semifinals",
